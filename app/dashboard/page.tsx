...
export default async function DashboardPage() {
  ...
  return (
    <div className="pb-12">
      {/* Welcome Banner */}
      <div className="relative mb-8 rounded-3xl overflow-hidden bg-gradient-to-br from-green-600 via-green-500 to-emerald-400 p-8 shadow-lg">
        ...
        <div className="relative flex items-center justify-between gap-6">
          ...
          <div className="hidden md:flex items-center gap-3">
            <Link
              href="/dashboard/widgets"
              className="px-5 py-2.5 bg-white text-green-700 rounded-xl text-sm font-semibold shadow hover:shadow-md transition-all hover:scale-105"
            >
              + Buat Widget
            </Link>
            <Link
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
        {stats.map((stat) => (
          ...
          >
            ...
            <div className="text-3xl font-bold mb-1 group-hover:text-green-600 transition-colors">{stat.value}</div>
      ...
      <div className="grid lg:grid-cols-3 gap-6 mb-8">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl border border-border p-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <Link href="/dashboard/analytics" className="text-xs text-green-600 font-medium hover:underline">
              Lihat detail →
            </Link>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-border p-6 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-3 flex-1">
            <Link
              href="/dashboard/widgets"
              className="flex items-center gap-3 p-3.5 rounded-xl bg-green-50 hover:bg-green-100 transition-colors group"
            >
              ...
              <div>
                <p className="text-sm font-semibold text-green-800">Buat Widget Baru</p>
                <p className="text-xs text-green-600">Tambah chatbot AI baru</p>
              </div>
            </Link>
            <Link
              href="/dashboard/widgets"
            ...
            >
              ...
              <div>
                <p className="text-sm font-semibold text-blue-800">Upload Dokumen</p>
                <p className="text-xs text-blue-600">Tambah ke knowledge base</p>
              </div>
            </Link>
            <Link
              href="/dashboard/analytics"
              className="flex items-center gap-3 p-3.5 rounded-xl bg-purple-50 hover:bg-purple-100 transition-colors group"
            >
              ...
              <div>
                <p className="text-sm font-semibold text-purple-800">Lihat Analytics</p>
                <p className="text-xs text-purple-600">Pantau performa chatbot</p>
              </div>
            </Link>
            <Link
            ...
            >
              ...
              <div>
                <p className="text-sm font-semibold text-gray-700">Pengaturan</p>
                <p className="text-xs text-gray-500">Konfigurasi akun & API</p>
              </div>
            </Link>
          </div>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-border p-6 mb-6">
        ...
        {recentWidgets.length === 0 ? (
          <div className="text-center py-10">
            ...
            <Link
              href="/dashboard/widgets"
              className="inline-flex items-center gap-2 px-5 py-2.5 gradient-bg text-white rounded-xl text-sm font-semibold hover:opacity-90 transition-opacity"
            >
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {recentWidgets.slice(0, 6).map((widget) => (
              <Link
                key={widget.id}
                href="/dashboard/widgets"
                className="flex items-center gap-3 p-4 rounded-xl border border-gray-100 hover:border-green-200 hover:bg-green-50/40 transition-all group"
              >
                <div
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-semibold truncate group-hover:text-green-700 transition-colors">{widget.name}</p>
                  <div className="flex items-center gap-1.5 mt-0.5">
                    <span className="w-1.5 h-1.5 bg-green-400 rounded-full" />
                    <span className="text-xs text-muted-foreground">Aktif</span>
                  </div>
                </div>
                <svg className="w-4 h-4 text-muted-foreground/40 group-hover:text-green-500 transition-colors flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            {recentWidgets.length > 6 && (
              <Link
                href="/dashboard/widgets"
                className="flex items-center justify-center gap-2 p-4 rounded-xl border border-dashed border-gray-200 hover:border-green-300 hover:bg-green-50/40 transition-all text-sm text-muted-foreground hover:text-green-600 font-medium"
              >
